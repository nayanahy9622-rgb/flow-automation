...
async function workspaceId():Promise<string>{try{const c=await cookies();return c.get("tentran_workspace")?.value||"anonymous";}catch{return "anonymous";}}
...
async function blobLoad(workspace:string):Promise<Record<string,ConnectorAuth>>{
 const token=blobToken();if(!token)return localLoad();
 const parts=token.split("_");const storeId=parts.length>=5&&parts[0]==="vercel"&&parts[1]==="blob"?parts[3]:"";
 const pathname=blobPath(workspace);
 const url=storeId?`https://${storeId}.private.blob.vercel-storage.com/${pathname}`:`${BLOB_API}?url=${encodeURIComponent(pathname)}`;
 const res=await fetch(url,{headers:blobHeaders(token),cache:"no-store"});
 if(res.status===404)return{};
 if(!res.ok)throw new Error(`Connector storage read failed (${res.status})`);
 const text=await res.text();return JSON.parse(decrypt(text));
}

async function blobSave(workspace:string,data:Record<string,ConnectorAuth>):Promise<void>{
 const token=blobToken();if(!token){localSave(data);return;}
 const pathname=blobPath(workspace);
 const res=await fetch(`${BLOB_API}?pathname=${encodeURIComponent(pathname)}`,{method:"PUT",headers:{...blobHeaders(token),access:"private","x-add-random-suffix":"0","x-allow-overwrite":"1","x-content-type":"application/json"},body:encrypt(JSON.stringify(data)),cache:"no-store"});
 if(!res.ok)throw new Error(`Connector storage write failed (${res.status})`);
}
