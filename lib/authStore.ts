...
export async function findUserByEmail(email:string){const users=await load<Record<string,User>>("tentran/auth/users.json","users.json");return Object.values(users).find(u=>u.email===email.trim().toLowerCase())||null;}
export async function authenticate(email:string,password:string){const user=await findUserByEmail(email);if(!user||!checkPassword(password,user.passwordHash))return null;return user;}
