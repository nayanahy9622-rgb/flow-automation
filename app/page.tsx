...
const recipes=[
  {title:"Recover abandoned carts",trigger:"Checkout abandoned",when:"Customer has an open cart and no order",act:"Send recovery message â†’ wait â†’ retry",impact:"Revenue recovery"},
  {title:"Win back high-value customers",trigger:"No purchase for 60 days",when:"Customer is high-LTV or recently engaged",act:"Personalized offer â†’ message â†’ measure",impact:"Retention"},
  {title:"Protect low stock",trigger:"Stock below threshold",when:"Sell-through is high or replenishment is delayed",act:"Alert â†’ reserve â†’ notify team",impact:"Availability"},
  {title:"Recover failed payments",trigger:"Payment failed",when:"Order is unpaid and recoverable",act:"Retry â†’ remind â†’ escalate",impact:"Cash recovery"},
  {title:"Launch new collection",trigger:"Collection published",when:"New products are ready to promote",act:"Segment â†’ publish campaign â†’ measure",impact:"Demand"},
];
...
export default function App(){
  ...
  return <div className="app">
    <aside>
      <div className="logo"><span>â—†</span> FlowOS</div>
      <div className="workspace"><div>COMMERCE CONTROL PLANE</div><b>âŒ„</b></div>
    ...
    <main>
      <header>
        ...
        <div className="head">
          <div className="search"><Search size={15}/><input placeholder="Search sources, orders, customersâ€¦" value={query} onChange={e=>setQuery(e.target.value)}/></div>
...
function subtitle(active:string){const m:Record<string,string>={Dashboard:"See what is happening, what matters next, and what FlowOS can execute.",Sales:"Revenue and order operations across every connected commerce source.",Customers:"Know who bought, who is at risk, and who should be contacted next.",Inventory:"Availability, sell-through and replenishment signals.",Finance:"Money in, money out, recovery and reconciliation.",Marketing:"Campaigns, audiences, acquisition and performance.",Data:"One data plane for every connected source and canonical business object.",Automations:"Reusable trigger â†’ context â†’ decision â†’ action workflows.","AI Agent":"An execution layer that observes signals, reasons with context and acts within policy.",Activity:"A single log of what FlowOS is doing and why.",Connectors:"Connect the systems that create and consume your commerce data.","API & Webhooks":"Inbound events, outbound actions, verification and automation triggers.",Settings:"Workspace defaults, data policy and operating controls."};return m[active]||"FlowOS commerce operations"}
...
function Dashboard({live,sources,syncing,refresh,go}:{live:Record<string,LiveBundle>;sources:Source[];syncing:boolean;refresh:()=>void;go:(v:string)=>void}){
  ...
  return <>
    ...
    <div className="stats">
      ...
      <Metric label="Orders sampled" value={orders.length?orders.length.toLocaleString():"â€”"} note="Current live pull"/>
      <Metric label="Average order value" value={aov?money(aov,orders[0]?.totalPriceSet?.shopMoney?.currencyCode):"â€”"} note={orders.length?"Derived from pulled orders":"Available after sync"}/>
    ...
    <div className="grid2">
      ...
      <section className="panel"><div className="panelHead"><div><h3>Data â†’ decision â†’ action</h3><p>The core automation loop.</p></div><Gauge size={18}/></div><Pipeline/></section>
    </div>
...
function Sales({live,sources,syncing,refresh,go}:{live:Record<string,LiveBundle>;sources:Source[];syncing:boolean;refresh:()=>void;go:(v:string)=>void}){
  ...
  return <>
    ...
    <div className="stats"><Metric label="Revenue sampled" value={revenue?money(revenue,currency):"â€”"} note="From current order pull"/><Metric label="Orders" value={orders.length||"â€”"} note="Recent order records"/><Metric label="Paid" value={orders.length?paid:"â€”"} note="Financial status"/><Metric label="Fulfilled" value={orders.length?fulfilled:"â€”"} note="Fulfillment status"/></div>
    <div className="grid2">
      <section className="panel"><div className="panelHead"><div><h3>Orders</h3><p>Canonical sales view built from the connected commerce source.</p></div><ShoppingBag size={18}/></div>{orders.length?<table><thead><tr><th>ORDER</th><th>STATUS</th><th>FULFILLMENT</th><th>VALUE</th></tr></thead><tbody>{orders.slice(0,20).map((o:any)=><tr key={o.id}><td><b>{o.name||o.id}</b></td><td>{o.displayFinancialStatus||"â€”"}</td><td>{o.displayFulfillmentStatus||"â€”"}</td><td>{o.totalPriceSet?.shopMoney?money(o.totalPriceSet.shopMoney.amount,o.totalPriceSet.shopMoney.currencyCode):"â€”"}</td></tr>)}</tbody></table>:<EmptyState title="Connect commerce data" copy="Sales will automatically become a live view once a commerce connector has orders to pull." onClick={()=>go("Connectors")}/>}</section>
...
function AutomationPage({open}:{open:()=>void}){return <>
  <div className="topbar"><div><div className="eyebrow">AUTOMATION ENGINE</div><h2>From event to outcome</h2><p>Keep the mental model simple: trigger â†’ context â†’ decision â†’ action â†’ measurement.</p></div><button className="primary" onClick={open}><Workflow size={14}/> New automation</button></div>
...
function AgentPage(){return <>
  ...
  <div className="grid2"><section className="panel"><div className="panelHead"><div><h3>Observe</h3><p>Signals arriving from connected systems.</p></div><Activity size={18}/></div>{["Order event","Customer lifecycle change","Inventory threshold","Campaign performance change"].map((x,i)=><div className="agentRow" key={x}><span>{String(i+1).padStart(2,"0")}</span><b>{x}</b><small>Available to agent context</small></div>)}</section><section className="panel"><div className="panelHead"><div><h3>Decide</h3><p>Reason over canonical context and business policy.</p></div><Bot size={18}/></div>{["Score opportunity","Select next best action","Check policy limits","Request approval when needed"].map((x,i)=><div className="agentRow" key={x}><span>â†’</span><b>{x}</b><small>{i<3?"Can be automated":"Human-in-the-loop"}</small></div>)}</section></div>
</>}
...
function Opportunity({recipe,onClick}:{recipe:any;onClick:()=>void}){return <button className="opportunity" onClick={onClick}><div className="opIcon"><Sparkles size={13}/></div><div><b>{recipe.title}</b><small>{recipe.trigger} Â· {recipe.impact}</small></div><ChevronRight size={14}/></button>}
...
function SourceCard({source,live}:{source:Source;live?:LiveBundle}){const count=countRecords(source.id,live);return <div className="sourceCard"><div className="sourceTitle"><div className="connectorIcon">{source.name.slice(0,1)}</div><div><b>{source.name}</b><span>{source.category}</span></div><span className={`status ${source.status}`}>{source.status}</span></div><p>{source.description}</p><div className="sourceMeta"><span>{live?"Live pull complete":"Not pulled in this view"}</span><b>{count!=null?count:"â€”"}</b></div></div>}
...
function money(v:any,currency?:string){const n=Number(v||0);if(!Number.isFinite(n))return "â€”";try{return new Intl.NumberFormat("en-IN",{style:"currency",currency:currency||"INR",maximumFractionDigits:2}).format(n)}catch{return `â‚¹${n.toLocaleString("en-IN")}`}}
function BellDot(){return <div className="bellDot" aria-label="Notifications"><span/></div>}
function CheckCircle2Icon(){return <div className="okMark">âœ“</div>}
