...
function FinanceModule({live,syncing,refresh,go}:{live?:LiveBundle;syncing:boolean;refresh:()=>void;go:(v:string)=>void}){
  ...
  return <>
    ...
    <div className="grid2">
      <section className="panel"><div className="panelHead"><div><h3>Payments</h3><p>Real records returned by the connected Razorpay account.</p></div><BadgeIndianRupee size={18}/></div>{payments.length?<table><thead><tr><th>PAYMENT</th><th>STATUS</th><th>METHOD</th><th>AMOUNT</th></tr></thead><tbody>{payments.slice(0,20).map((p:any)=><tr key={p.id}><td><b>{p.id}</b></td><td>{p.status||"—"}</td><td>{p.method||"—"}</td><td>{fmt(Number(p.amount||0)/100)}</td></tr>)}</tbody></table>:<div className="empty"><Database size={18}/><b>{hasLive?"No Razorpay payments yet":"Razorpay is not loaded yet"}</b><span>{hasLive?"Your credentials are valid, but this test account has no payment records yet.":"Connect Razorpay to pull payment records."}</span><button className="secondary" suppressHydrationWarning onClick={()=>go("Connectors")}>Open connectors</button></div>}</section>
